...
def run(path=None, generate_pathway=False, pilot='', service=''):
    metadata = pilot + ' - ' + service + ' - ' + os.path.basename(path)
    ...
    document = document_exists(metadata, project['id'])
    
    if document:
        # TODO: METHOD TO CONVERT IN DOCCANO_DICT 
        return 'already exixsts'
    ...
    doccano_dict, ner_path = annotator.export_to_doccano(ner_dict, path, pilot, service)
    ...
    if generate_pathway:
        metadata = pilot + ' - ' + service + ' - ' + 'Where - ' + os.path.basename(path)
        project = get_project_by_name('easyRights Pathway')

        document = document_exists(metadata, project['id'])

        if document:
            #TODO RETURN ALL THREE DOCUMENT OF PATHWAY
            return 'todo'
        ...
        pathway, pathway_path = pathway_to_doccano(json.loads(json_pathway), path, pilot, service)

        try:
            doccano_client.post_doc_upload(project_id=project['id'], file_format='json', file_name=pathway_path)
        except json.decoder.JSONDecodeError:
            pass
